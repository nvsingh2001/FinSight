...
def answered_via(route, year):
    """Caption for a finished answer. The fiscal year only narrows vector
    retrieval, so it is noise on a graph-only answer."""
...
st.title("FinSight")
st.caption(
    "Ask about the last five 10-K filings of Apple, Microsoft, and NVIDIA, plus "
    "19 fiscal years of reported financials. Quantitative questions hit the "
    "knowledge graph, narrative ones the vector store. Name a fiscal year and "
    "the filings for that year are the ones that get read."
)
